...
class User(BaseModel, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 用户ID、编号
    ...
    house = db.relationship('House', backref='user')  # 用户发布的房屋
    orders = db.relationship('Order', backref='user')  # 用户下的订单

    # 通过property装饰器可以将方法转化为属性
    @property
    def password(self):
        """读取属性的函数行为"""
        # return ''
        # 如果不需要对这个属性设置值，可以通过raise抛出AttributeError异常的方式来实现
...
class House(BaseModel, db.Model):
    ...
    address = db.Column(db.String(512), default='')  # 房屋具体地址
...
class Order(BaseModel, db.Model):
    ...
    house_id = db.Column(db.Integer, db.ForeignKey('ih_house_info.id'), nullable=False)  # 预定的房间编号
    ...
    trade_no = db.Column(db.String(128))  # 交易的流水号 支付宝
