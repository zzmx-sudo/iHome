...
@api.route("/facilitys", methods=["GET"])
def get_facilities():
    """获取房屋设施信息"""
    # 尝试从redis中读取数据
    try:
        resp_json = redis_store.get("facility_info")
    except Exception as e:
        current_app.logger.error(e)
    else:
        if resp_json is not None:
    ...
    # 查询数据库，读取设施信息
    try:
        facility_li = Facility.query.all()
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg=u'查询数据库异常')
    ...
    facility_dict_li = []
    # 将对象转换为字典
    for facility in facility_li:
        facility_dict_li.append(facility.to_dict())

    # 将数据保存到redis中
    # 先将数据转换为json字符串
    resp_dict = dict(errno=RET.OK, errmsg='OK', data=facility_dict_li)
    resp_json = json.dumps(resp_dict)
    try:
        redis_store.setex("facility_info", constants.FACILITY_INFO_REDIS_CACHE_EXPIRS, resp_json)
    except Exception as e:
        current_app.logger.error(e)
...
def get_user_houses():
    ...
    if house:
        for hou in house:
            houses_list.append(hou.to_basic_dict())
...
def get_house_index():
    ...
    if ret:
        ...
        try:
            # 查询数据库，返回房屋订单数目最多5条数据
            houses = House.query.order_by(House.order_count.desc()).limit(constants.HOME_PAGE_MAX_HOUSE)
        except Exception as e:
            current_app.logger.error(e)
...
def get_house_detail(house_id):
    """获取房屋详情"""
    # 前端在房屋详情页面展示时，如果浏览器页面的用户表示该房屋的房东，则展示预定按钮，否则不展示
    # 所以需要后端返回登录用户的user_id
    # 尝试获取用户登录的信息，如登录，则返回给前端登录用户的user_id，否则返回user_id=-1
    ...
    try:
        ...
        ret = None
    if ret:
        # current_app.logger.error("hit house info redis")
...
# GET /api/v1.0/houses?ad=2020-06-01&ed=2020-06-20&aid=10&sk=new&p=1
@api.route("/houses", )
def get_house_list():
    """获取房屋的列表信息（搜索页面）"""
    ...
    # 判断区域id
    # 如果存在区域id，再从模型类中获得对应的对象。如果不加判断，区域id为空时会执行异常后的代码
    if area_id:
        try:
            area = Area.query.get(area_id)
        except Exception as e:
    ...
    try:
        if start_data and end_data:
            # 查询冲突的订单
            # 起始时间和结束时间都有值时，冲突的订单的起始时间小于等于结束时间，冲突订单的结束时间大于等于起始时间。
            conflict_orders = Order.query.filter(Order.begin_date <= end_data and Order.end_date >= start_data).all()
        elif start_data:
    ...
    if conflict_orders:
        ...
        # 如果冲突的房屋id不为空，向过滤条件容器中添加条件，notin表示房屋ID不在后面的列表中
